...
import { Play, Pause, RotateCcw, Sliders, TrendingDown, Award, CheckCircle } from 'lucide-react';
    ...
    if (isTraining) {
      ...
      trainingInterval.current = setInterval(() => {
        setCurrentEpoch((prevEpoch) => {
          const nextEpoch = prevEpoch + 5;
          if (nextEpoch >= epochs) {
            clearInterval(trainingInterval.current);
            setIsTraining(false);
            setConvergenceState('Optimal');
            setTrainLogs((prev) => [
              ...prev,
      ...
      <div className="lg:col-span-4 bg-white border border-outline-variant p-6 rounded shadow-sm flex flex-col justify-between">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  disabled={isTraining}
                  className={`p-2 rounded border text-center transition-all duration-200 cursor-pointer disabled:opacity-50 text-xs font-semibold ${
                    modelType === 'LSTM'
              ...
              <div className="flex justify-between gap-1">
                {[1, 2, 3, 4].map((num) => (
                  <button
                    ...
                    disabled={isTraining}
                    className={`flex-1 py-1.5 rounded border text-xs font-mono font-bold transition-all duration-200 cursor-pointer ${
                      hiddenLayers === num
        ...
        <div className="mt-8 pt-4 border-t border-outline-variant flex gap-3">
          <button
            ...
            className={`flex-1 font-label-md text-xs font-bold py-3 px-4 rounded-lg flex items-center justify-center space-x-2 transition-all cursor-pointer shadow ${
              isTraining
